...
|---|---|---|
| Agent Context Compaction with Query-Aware Clustering, September 2026 | Explores query-aware selection/compaction rather than treating all history as equally useful.[18] | Compare next-decision-aware evidence selection with a simple recent-tail baseline; retain access to exact source records. |
| ARC-Bench, September 2026 | Evaluates context compression for subsequent agent tool use, rather than summary readability alone.[19] | Score operation selection, exact arguments and constraint retention after compaction. |
| The Long-Context Cliff, August 2026 | Studies degradation associated with long context; reported results are workload/model-specific.[20] | Sweep context occupancy and distractor density on endpoint tasks instead of assuming advertised capacity means reliable use. |
| Context as a Control Plane, July 2026 | Studies agent context management as an explicit control problem.[21] | Compare context policies as versioned internal strategies; do not make model context the authority control plane. |
